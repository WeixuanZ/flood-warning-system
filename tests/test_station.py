...
class TestClass:
    ...
    def test_typical_range_consistent(self):
        assert (
            MonitoringStation(
                "111", "111", "station1", (0.0, 1.0), (1.0, 4.0), "River 1", "Town 1"
            ).typical_range_consistent()
            == True
        )
        assert (
            MonitoringStation(
                "111", "111", "station1", (0.0, 1.0), None, "River 1", "Town 1"
            ).typical_range_consistent()
            == False
        )
        assert (
            MonitoringStation(
                "111", "111", "station1", (0.0, 1.0), (1, -4), "River 1", "Town 1"
            ).typical_range_consistent()
            == False
        )
        assert (
            MonitoringStation(
                "111", "111", "station1", (0.0, 1.0), (0.5, 0.2), "River 1", "Town 1"
            ).typical_range_consistent()
            == False
        )
    ...
    def test_inconsistent_typical_range_stations(self):
        ...
        assert inconsistent_typical_range_stations([station1, station2]) == [
            station2.name
        ]
        assert inconsistent_typical_range_stations([station2, station3]) == [
            station2.name,
            station3.name,
        ]
        assert inconsistent_typical_range_stations([station4, station2, station3]) == [
            station2.name,
            station3.name,
            station4.name,
        ]
    ...
    def test_relative_water_level(self):
        ...
        station2 = MonitoringStation(
            ...
            coord=(1.0, 1.0),
            typical_range=(1.0, 0.0),
            river="test_river_2",
            town="test_town_2",
        )
        assert station1.relative_water_level() is None
        station1.latest_level = 0.0
        assert station1.relative_water_level() == 0.0
        ...
        station1.latest_level = 2.0
        assert station1.relative_water_level() == 2.0
